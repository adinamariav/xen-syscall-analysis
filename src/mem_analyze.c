...
void process_syscall_analyze_mem(vmi_instance_t vmi, vmi_event_t* event) {
    reg_t rdi, rax, cr3;
    vmi_pid_t pid = -1;
    FILE* fp = NULL;
...
event_response_t syscall_cb_analyze_mem(vmi_instance_t vmi, vmi_event_t *event) {
    ...
    process_syscall_analyze_mem(vmi, event);
...
event_response_t cr3_register_analyze_callback(vmi_instance_t vmi, vmi_event_t *event) {
    if ( !mem_events_registered )
        mem_events_registered = register_lstar_mem_event(vmi, event, syscall_cb_analyze_mem);
    vmi_clear_event(vmi, &cr3_event, NULL);
    return 0;
}


event_response_t register_mem_events_analyze(vmi_instance_t vmi) {
    memset(&cr3_event, 0, sizeof(vmi_event_t));
    cr3_event.version = VMI_EVENTS_VERSION;
    cr3_event.type = VMI_EVENT_REGISTER;
    cr3_event.callback = cr3_register_analyze_callback;
...
int mem_analyze (char *name, int set_time, int window_size) {
    ...
    if ( VMI_SUCCESS == register_mem_events_analyze(vmi) ) {
        while (!interrupted) {
